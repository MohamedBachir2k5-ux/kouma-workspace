    ...
    <div ref={ref} className="relative">
      {selected ? (
        <div className="flex items-center gap-3 px-4 py-3 bg-bg border border-indigo rounded-xl">
          <Avatar firstName={selected.firstName} lastName={selected.lastName} id={selected.id} size="sm" src={selected.avatarUrl} />
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold text-ink">{selected.firstName} {selected.lastName}</div>
            <div className="text-xs text-muted">{selected.jobTitle || t(selected.role === 'admin' ? 'common.administrator' : 'common.collaborator')}</div>
          </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={t('admin.teamsSearchResp')}
          ...
          {results.map(u => (
            <button key={u.id} type="button" onClick={() => select(u.id)}
              ...
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted">{u.jobTitle || t(u.role === 'admin' ? 'common.administrator' : 'common.collaborator')}</div>
              </div>
            </button>
          ))}
        </div>
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-md bg-surface rounded-2xl border border-border shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="p-6 border-b border-border">
          <h3 className="font-bold text-navy text-lg">{isEdit ? t('admin.teamsEditTitle') : t('admin.teamsNewTitle')}</h3>
        </div>

        <div className="p-6 space-y-5">
          <div>
            <label className="block text-xs font-semibold text-ink mb-2 uppercase tracking-wide">{t('admin.teamsNameLabel')} *</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-ink mb-2 uppercase tracking-wide">{t('admin.teamsDescLabel')}</label>
            <textarea value={form.description} onChange={e => update('description', e.target.value)}
              placeholder={t('admin.teamsDescPlaceholder')} rows={2}
          ...
          <div>
            <label className="block text-xs font-semibold text-ink mb-2 uppercase tracking-wide">{t('admin.teamsColorLabel')}</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-ink mb-2 uppercase tracking-wide">{t('admin.teamsResponsableLabel')} *</label>
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-ink">{t('admin.teamsTitle')}</h1>
          <p className="text-sm text-muted mt-0.5">{t(teams.length !== 1 ? 'admin.teamsCountPlural' : 'admin.teamsCount', { count: teams.length })}</p>
        </div>
        <button onClick={() => { setEditTeam(null); setShowModal(true) }}
          ...
          <Plus size={16} /> <span className="hidden sm:inline">{t('admin.teamsNewTitle')}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key={team.id} className="bg-surface rounded-xl border border-border p-5 hover:border-indigo/20 hover:shadow-sm transition-all relative">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="relative">
                  <button onClick={() => setMenuOpen(menuOpen === team.id ? null : team.id)}
                    ...
                    className="w-12 h-12 flex items-center justify-center rounded-lg text-muted hover:text-ink hover:bg-bg transition-colors">
              ...
              {responsable && (
                <div className="flex items-center gap-2 mb-3 text-xs text-muted">
                  <Avatar firstName={responsable.firstName} lastName={responsable.lastName} id={responsable.id} size="sm" src={responsable.avatarUrl} />
                  <span>{t('admin.teamsResponsableOf')} <span className="font-medium text-ink">{responsable.firstName} {responsable.lastName}</span></span>
                </div>
              )}
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="flex items-center gap-1.5 text-xs text-muted">
                  <Users size={13} />
                  {t(members.length !== 1 ? 'admin.memberCountPlural' : 'admin.memberCount', { count: members.length })}
                </span>
              </div>
            </div>
          )
        ...
        <button onClick={() => { setEditTeam(null); setShowModal(true) }}
          ...
          <span className="text-sm text-muted">{t('admin.teamsNewTitle')}</span>
        </button>
      </div>
