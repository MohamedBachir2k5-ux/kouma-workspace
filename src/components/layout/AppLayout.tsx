    ...
    <nav className="md:hidden flex items-stretch justify-around h-16 bg-surface border-t border-border shrink-0 px-2 safe-area-bottom relative">
      {NAV_ITEMS_MOBILE_MAIN.map(({ to, icon: Icon, key }) => (
        <NavLink
          ...
          to={to}
          className={({ isActive }) =>
            `flex flex-col items-center justify-center gap-0.5 px-3 py-1 min-h-[48px] rounded-lg transition-colors ${
      ...
      <div ref={moreRef} className="relative flex items-center">
        <button
          ...
          className={`flex flex-col items-center justify-center gap-0.5 px-3 py-1 min-h-[48px] rounded-lg transition-colors ${showMore ? 'text-indigo' : 'text-faint'}`}
        >
  ...
  return (
    <LayoutProvider>
      <div className="flex flex-col h-dvh bg-bg overflow-hidden">
        ...
        {/* Bottom nav — mobile, hidden when inside a conversation */}
        <BottomNav />
      </div>
    </LayoutProvider>
  )
